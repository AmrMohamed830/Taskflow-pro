...
import type { UpdateTaskStatusData, Task, TasksResponse } from "../types/tasks";
...
export const useUpdateTaskStatus = () => {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, data }) => {
      ...
      const previousTasksData = queryClient.getQueriesData<TasksCacheData>({ queryKey: ["tasks"] });

      queryClient.setQueriesData<TasksCacheData>({ queryKey: ["tasks"] }, (oldData: TasksCacheData | undefined) => {
        if (!oldData) return oldData;
        if (!Array.isArray(oldData) && "tasks" in oldData && Array.isArray(oldData.tasks)) {
          return {
            ...oldData,
